...
var get_city=function(province_sel,city_sel){
	var city_obj={
	...
	var province=Object.keys(city_obj);
	province_sel.html('<option>请选择</option>');
	for(var i=0,l=province.length;i<l;i++){
		province_sel.append('<option>'+province[i]+'</option>');
	}
	province_sel.on('change',function(){
		city_sel.html('<option>请选择</option>');
		var sel=$(this).find('option:selected').text();
		var city=city_obj[sel];
		for(var i=0,l=city.length;i<l;i++){
			city_sel.append('<option>'+city[i]+'</option>')
		}
	})
}